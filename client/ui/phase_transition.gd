...
func _ready() -> void:
	...
	var banner: Label = UiKit.label(self, "FASE %d CONCLUÍDA!" % index, Rect2(0, 40, 1280, 80), 56, Color("ffd04a"), Color("5a2408"), HORIZONTAL_ALIGNMENT_CENTER)
	banner.add_theme_constant_override("outline_size", 14)
	banner.name = "Banner"
	UiKit.label(self, str(report.get("cleared", "")), Rect2(0, 114, 1280, 32), 22, Color("fff0d0"), UiKit.INK, HORIZONTAL_ALIGNMENT_CENTER)
	# Next phase card
	var card: Panel = UiKit.panel(self, Rect2(170, 170, 520, 400), "wood")
	UiKit.panel(card, Rect2(14, 14, 492, 372), "paper")
	UiKit.title(card, "PRÓXIMA: FASE %d DE %d" % [index + 1, count], Rect2(0, 22, 520, 34), 24)
	...
	UiKit.label(card, str(next.get("name", "")), Rect2(0, 262, 520, 40), 30, Color("c0402f") if boss_phase else Color("5a2e10"), Color("fff0d0"), HORIZONTAL_ALIGNMENT_CENTER)
	var goal: String = "Chefão! Derrote-o para abrir o baú." if boss_phase else "Derrote o guardião e os lacaios."
	match str(next.get("objective", "defeat")):
		"totems":
			goal = "Objetivo: destrua os cristais."
		"survive":
			goal = "Objetivo: sobreviva %d turnos." % int(next.get("turns", 5))
	UiKit.label(card, goal, Rect2(20, 302, 480, 30), 18, UiKit.TEXT_DARK, Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)
	UiKit.label(card, "Vida +%d%%  •  POW mantido  •  Quem caiu volta com %d%% de vida" % [roundi(float(app.balance.pve.phase_heal) * 100.0), roundi(float(app.balance.pve.revive_hp) * 100.0)], Rect2(20, 336, 480, 30), 14, Color("2f6a1f"), Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)
	# Drops of this phase
	var loot: Panel = UiKit.panel(self, Rect2(716, 170, 400, 400), "wood_dark")
	UiKit.title(loot, "ENCONTRADO", Rect2(0, 12, 400, 34), 24)
	UiKit.art(loot, "res://assets/items/moeda.png", Rect2(40, 62, 40, 40))
	UiKit.label(loot, "+%d moedas" % int(report.get("gold", 0)), Rect2(90, 62, 280, 40), 24, Color("ffd46b"), UiKit.INK)
	var maps: Array = report.get("maps", [])
	if maps.is_empty():
		UiKit.label(loot, "Nenhum mapa caiu nesta fase.", Rect2(20, 130, 360, 30), 16, Color("c8b8a0"), UiKit.INK, HORIZONTAL_ALIGNMENT_CENTER)
	for i in range(mini(maps.size(), 4)):
		...
		UiKit.label(row, "%s  •  %d atributo(s)" % [InstanceRun.quality_label(str(item.quality)), item.mods.size()], Rect2(60, 30, 296, 24), 13, Color("e0d0b0"), UiKit.INK)
		row.tooltip_text = "\n".join(InstanceRun.describe_map(item))
	UiKit.label(loot, "Os mapas já estão na Mochila (aba Mapas).", Rect2(10, 360, 380, 28), 13, Color("c8b8a0"), UiKit.INK, HORIZONTAL_ALIGNMENT_CENTER)
	var go: Button = UiKit.button(self, "AVANÇAR ▶", Rect2(540, 600, 200, 52), advance, "button_green", 22)
	go.name = "Advance"
