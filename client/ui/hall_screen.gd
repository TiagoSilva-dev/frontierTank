...
func _ready() -> void:
	...
	UiKit.title(self, tr("Salão de jogos"), Rect2(318, 26, 280, 46), 24)
	UiKit.label(self, tr("Lista de salas"), Rect2(26, 78, 150, 30), 18, Color.WHITE, UiKit.INK)
	var modes: OptionButton = OptionButton.new()
	for text: String in ["Todas as modalidades", "Combate Livre", "Aguardando jogadores"]:  # i18n
		modes.add_item(tr(text))
...
func build_rooms() -> void:
	...
	page_label.text = tr("Página %d/%d  •  %d salas aguardando  •  %d em jogo") % [page + 1, pages, waiting, rooms.size() - waiting]
...
func build_actions() -> void:
	...
	UiKit.label(hint, tr("Clique em \"Jogar\" para começar o combate"), Rect2(0, 0, 368, 28), 13, Color("fff4a0"), Color("5a1004"), HORIZONTAL_ALIGNMENT_CENTER)
	var actions: Array = [["team", "Equipe", create_team, "Criar uma sala com a sua equipe"], ["search", "Buscar", search_room, "Entrar numa sala pelo número"], ["play", "Jogar", quick_play, "Entrar na primeira sala disponível"]]  # i18n
	for i in range(3):
		var action: Array = actions[i]
		var button: Button = UiKit.icon_button(self, PixelIcons.get_icon(action[0]), Rect2(532 + i * 122, 546, 104, 104), action[2], tr(action[3]))
...
func search_room() -> void:
	var dialog: Control = UiKit.modal(self, tr("BUSCAR SALA"), tr("Digite o número da sala:"))
	...
	var submit: Callable = func() -> void:
		...
		dialog.queue_free()
		if room.is_empty():
			UiKit.notice(self, tr("BUSCAR SALA"), tr("Sala %s não encontrada neste canal.") % field.text)
		else:
			try_join(room)
	field.text_submitted.connect(func(_text: String) -> void: submit.call())
	UiKit.button(dialog, tr("ENTRAR"), Rect2(rect.position.x + 90, rect.end.y - 60, 150, 42), submit)
	UiKit.button(dialog, tr("CANCELAR"), Rect2(rect.end.x - 240, rect.end.y - 60, 150, 42), dialog.queue_free)
