...
static func open(parent: Node, app: Node, on_done: Callable = Callable()) -> Control:
	var root: Control = UiKit.modal(parent, "RESGATAR CUPOM", "", Vector2(560, 320))
	var rect: Rect2 = root.get_meta("rect")
	UiKit.label(root, "Digite o código do cupom:", Rect2(rect.position.x + 40, rect.position.y + 60, 480, 30), 17, UiKit.TEXT_DARK)
	var field: LineEdit = LineEdit.new()
	field.name = "CouponField"
	field.position = rect.position + Vector2(40, 94)
	field.size = Vector2(480, 42)
	field.max_length = 24
	field.placeholder_text = "Ex.: TESTARTUDO"
	field.add_theme_font_override("font", UiKit.font())
	field.add_theme_font_size_override("font_size", UiKit.fs(22))
	root.add_child(field)
	var result: Label = UiKit.label(root, "Para testes: TESTARTUDO libera todas as armas e cosméticos; AURAS mostra as quatro auras.", Rect2(rect.position.x + 40, rect.position.y + 142, 480, 104), 14, Color("6a4a2a"), Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)
	...
	UiKit.button(root, "RESGATAR", Rect2(rect.position.x + 110, rect.end.y - 62, 160, 44), redeem, "button_green")
	UiKit.button(root, "FECHAR", Rect2(rect.end.x - 270, rect.end.y - 62, 160, 44), root.queue_free)
