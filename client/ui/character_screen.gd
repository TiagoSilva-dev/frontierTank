...
const ITEM_NAMES: Dictionary = {
	"pedra_fortalecimento": ["Pedra de Fortalecimento I", "res://assets/items/pedra_i.png"],
	"strength_stone_ii": ["Pedra de Fortalecimento II", "res://assets/items/pedra_ii.png"],
	"strength_stone_iii": ["Pedra de Fortalecimento III", "res://assets/items/pedra_iii.png"],
	"strength_stone_iv": ["Pedra de Fortalecimento IV", "res://assets/items/pedra_iv.png"],
...
func build() -> void:
	...
	UiKit.label(contents, "MOCHILA", Rect2(44, 18, 220, 64), 34, Color("ffd681"), UiKit.INK)
	UiKit.button(contents, "VOLTAR", Rect2(290, 26, 130, 48), app.close_bag)
	UiKit.button(contents, "FERREIRO", Rect2(430, 26, 150, 48), open_smith, "button_blue")
	UiKit.button(contents, "LOJA", Rect2(590, 26, 120, 48), open_shop, "button_green")
	UiKit.button(contents, "CUPOM", Rect2(720, 26, 130, 48), open_coupon)
	...
	UiKit.label(contents, "moedas", Rect2(1078, 56, 160, 24), 15, Color("e0c89a"), UiKit.INK)
	build_profile()
	build_inventory()
...
func build_profile() -> void:
	...
	UiKit.label(contents, "Informações Pessoais", Rect2(44, 100, 360, 36), 24, Color("fff0c7"), UiKit.INK)
	for i in range(3):
		var name_text: String = ["Perfil", "Atributos", "Histórico"][i]
		UiKit.button(contents, name_text, Rect2(334 + i * 96, 102, 92, 32), select_tab.bind(name_text), "tab_active" if tab_name == name_text else "tab", 14)
	...
	UiKit.label(head, "Ranking %d" % app.profile.ranking(), Rect2(440, 0, 118, 40), 16, Color("ffb0a0"), UiKit.INK, HORIZONTAL_ALIGNMENT_RIGHT)
	if tab_name == "Perfil":
		build_equipment()
...
func build_equipment() -> void:
	...
	UiKit.label(pet, "Mascote", Rect2(-8, 50, 82, 16), 11, UiKit.TEXT_DARK, Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)
	UiKit.label(contents, "EXP", Rect2(52, 500, 56, 22), 16, Color("563b29"))
	...
	UiKit.label(contents, "%s  •  %s" % [aura_text("Aura da arma", int(look.weapon_level)), aura_text("Aura da roupa", int(look.clothes_level))], Rect2(48, 662, 564, 26), 14, UiKit.TEXT_DARK, Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)

func aura_text(label_text: String, level: int) -> String:
	var entry: Dictionary = Armory.aura(level)
	return "%s: %s" % [label_text, "%s (+%d)" % [entry.name.replace("Aura ", ""), level] if not entry.is_empty() else "nenhuma"]
...
func equipment_slot(slot: String, rect: Rect2) -> void:
	...
	button.tooltip_text = Armory.SLOT_NAMES[slot]
	if inst.is_empty():
		UiKit.label(button, Armory.SLOT_NAMES[slot], Rect2(0, 0, rect.size.x, rect.size.y), 13, Color("a8906c"), Color.TRANSPARENT, HORIZONTAL_ALIGNMENT_CENTER)
		return
...
func build_stats(rect: Rect2) -> void:
	...
	var left: Array = [["Ataque", "ataque"], ["Agilidade", "agilidade"], ["Defesa", "defesa"], ["Sorte", "sorte"]]
	for i in range(4):
		var x: float = 10 + (i % 2) * 150
		var y: float = 10 + (i / 2) * 52
		UiKit.label(box, left[i][0], Rect2(x, y, 76, 40), 15, Color("ffe6a0"), UiKit.INK)
	...
	var right: Array = [["Dano", "dano", Color("ff9a2a")], ["Proteção", "protecao", Color("3a8bff")], ["Vida", "vida", Color("e0302a")], ["Força física", "energia", Color("5cc83a")]]
	for i in range(4):
		var y: float = 6 + i * 27
		UiKit.label(box, right[i][0], Rect2(312, y, 100, 24), 14, Color("ffe6a0"), UiKit.INK)
...
func build_attributes() -> void:
	...
	UiKit.label(contents, "ATRIBUTOS DE COMBATE", Rect2(60, 196, 540, 34), 22, Color("624128"))
	var extra: Dictionary = numbers.extra
	var rows: Array[String] = [
		"Arma: %s  •  Dano %d  •  Cratera %d" % [weapon.name, int(weapon.damage), int(weapon.radius)],
		"Ângulo %d° a %d°  •  POW: %s" % [int(weapon.angle[0]), int(weapon.angle[1]), weapon.pow.name],
		"Ataque +%d dos itens: dano x%.2f" % [int(extra.ataque), Armory.attack_scale(extra)],
		"Defesa +%d dos itens: dano recebido x%.2f" % [int(extra.defesa), Armory.defense_scale(extra)],
		"Agilidade %d: define o Delay e quem começa" % int(numbers.agilidade),
		"Sorte +%d dos itens: %.1f%% de acerto crítico (x1,5)" % [int(extra.sorte), Armory.crit_chance(extra) * 100.0],
...
func build_inventory() -> void:
	...
	equip_button = UiKit.button(contents, "EQUIPAR", Rect2(670, 654, 180, 40), equip_selected, "button_green")
	sell_button = UiKit.button(contents, "VENDER", Rect2(858, 654, 130, 40), sell_selected)
	UiKit.button(contents, "FORTALECER", Rect2(996, 654, 238, 40), open_smith, "button_blue")
...
func describe(inst: Dictionary) -> String:
	...
	for key: String in Armory.ATTRS:
		if int(attrs[key]) > 0:
			parts.append("%s +%d" % [Armory.ATTR_NAMES[key], int(attrs[key])])
	match Armory.kind_of(id):
		"weapon":
			var weapon: Dictionary = Armory.build_weapon(inst)
			text += "  [color=#ffd46b]%s[/color]\nDano %d • Cratera %d • Ângulo %d°–%d° • POW: %s\n%s" % [Armory.quality_def(str(inst.quality)).label, int(weapon.damage), int(weapon.radius), int(weapon.angle[0]), int(weapon.angle[1]), weapon.pow.name, " • ".join(parts)]
		"aux":
			text += "\n%s" % Armory.aux_def(id).desc
		"cosmetic":
...
func refresh_selection(list: Array[Dictionary]) -> void:
	selected_label.text = "Selecione um item da mochila ou um espaço de equipamento."
	equip_button.disabled = true
	equip_button.text = "EQUIPAR"
	sell_button.disabled = true
	if selected.begins_with("slot:"):
		selected_label.text = "%s: nenhum item equipado." % Armory.SLOT_NAMES[selected.substr(5)]
		return
	if selected.begins_with("uid:"):
		...
		equip_button.text = "REMOVER" if equipped else "EQUIPAR"
