...
# the saved profile back. Not part of the offline suite: start the stack first
...
func run() -> void:
	...
	check(await app.auth.login(user, "senha-de-teste-1", true) == "", "an account is created on the API")
