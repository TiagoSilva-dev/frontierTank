# Frontier Tank: Nova Era — 0.13 (preparação do lançamento)
...
| **Cidade** (`docs/screens/city.png`) | Ilha com o Salão de Jogos (coliseu) no centro da praça e os prédios nos lotes em volta: Ferreiro, Instância, Leilão, Centro Comercial e Casa dos Mascotes; a capela do Namoro fica só como cenário (desligado por enquanto). Mar em movimento, fumaça da chaminé e brilho da forja, faíscas no coliseu, portal girando, brilhos nas lojas e gaivotas. Prédios clicáveis: **Ferreiro**, **Centro Comercial** e, online, **Leilão** abrem de verdade. Botões **CUPOM** e **MOCHILA**, alto-falante, canal, chat e barra SHOP · MOCHILA · PET · CORREIO · MISSÃO · AJUDA · SAIR. |
| **Mochila** (`bag.png`) | Informações Pessoais: slots Chapéu, Óculos, Cabelo, Roupa, Asas, Arma e Auxiliar em volta do personagem, que veste tudo o que está equipado, com a aura da arma atrás da cabeça e dos ombros. Ataque, Agilidade, Defesa, Sorte, Dano, Proteção, Vida e Força física. Inventário com Armas, Visual, Auxiliar, Materiais e **Mapas**; equipar, remover e vender. |
...
## POW de jogo de luta e monstros com habilidades (26/09/2026)
- **POW**: com a barra cheia ela fica dourada e pisca "POW 100%". Ao armar (B) a partida congela por um instante para o **corte de tela**: faixa diagonal com o seu personagem e o nome do especial em letras douradas. Na carga, círculos mágicos giram no chão e na frente da arma; no disparo, clarão; no impacto, pausa maior, clarão branco, tremor e o **dano total** do POW num estouro em quadrinho.
- **Monstros não atiram**: cada um usa habilidades próprias que **não erram** (a Defesa e o escudo continuam valendo). O nome aparece sobre o monstro e anéis vermelhos marcam quem vai ser atingido. Tipos: golpe num alvo, meteoro (os de fúria abrem uma cratera pequena), área em todos os heróis, corrente que pula de herói em herói e dreno que cura o monstro. Cada monstro tem a básica, uma que vem a cada 2–3 ataques e a de fúria.
- **Interface** mais redonda, com relevo e sombra; os espaços da mochila são rebaixados.
- Capturas: `docs/screens/pow_cutin.png`, `pow_charge.png`, `pow_total.png`, `ability_cast.png` e `ability_chain.png`.
...
## Verificação
Execute: `powershell -File tools/run.ps1 -Test` — 579 verificações (combate 54, instâncias, mapas e habilidades dos monstros 78, interface 78, armas/Ferreiro/cupons/especiais 56, POW e escala visual 45, atributos aleatórios e moedas 64, idiomas 25, leilão 62, lançamento 117: web, identidade, textos legais, consentimento, Steam e página da loja), mais as de rede: 58 de lockstep e operações e 151 com um servidor e jogadores de verdade (inclui o Leilão, o Correio, a exclusão de conta, a denúncia no chat e a loja Steam com um GodotSteam falso). A API em Go (contas, leilão, privacidade, denúncias e Steam contra uma Steam falsa) e as verificações contra a pilha no ar: `server/README.md`, seção Testes.
...
## Limites
No modo offline, salas, jogadores e chat do canal são simulados por IA e isso é avisado no chat; moedas e itens ficam em `user://profile.json` e não valem como economia online; Leilão e Correio só funcionam online. O Namoro está desligado (a capela é só cenário); PET (e a Casa dos Mascotes) e Missão mostram aviso de "ainda não disponível". A arte PixelLab que falta (molduras, retratos do POW, efeitos das habilidades) está planejada em `docs/PIXELLAB_PLANO.md`; até lá é desenhada por código. O leilão ainda não tem lances nem troca direta de moedas (Estrela ↔ Solar). Rosto e olhos ainda não são slots separados. A Steam precisa do App ID, da extensão GodotSteam e da configuração na Steamworks; estornos da Steam ainda não retiram o item (docs/STEAM.md, pendências). Os ícones das sete moedas da 0.10 são provisórios, desenhados por `tools/currency_icons.py`, até a arte PixelLab (`docs/PIXELLAB_0_10.md`).
