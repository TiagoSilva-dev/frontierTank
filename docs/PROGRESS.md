...
## Atual — visual e PvE (26/09/2026)
- **POW de jogo de luta**, nos 5 tempos da referência: a barra cheia vira ouro com brilho correndo e "POW 100%"; ao armar, a partida congela (1,05 s, igual para todos no online) para o **corte de tela** — tela escura, faixa diagonal com linhas de velocidade, o lutador com tudo equipado e o nome do especial em letras douradas; na carga, **círculos mágicos** girando no chão e na frente da arma; no disparo, clarão da cor da arma; no impacto, hit-stop maior, clarão branco, tremor e o **dano total** do POW num estouro em quadrinho que conta até o número. O estouro "POW!" antigo saiu.
- **Monstros com habilidades que não erram**: os 12 monstros deixaram de atirar. Cada um tem a sua (Ferrão Solar, Lança do Sol, Raio Solar, Chuva de Brasas, Fúria Solar, Rocha Ígnea, Tremor, Erupção, Chama das Máscaras que rouba vida, Dança das Chamas que pula entre os heróis, Mordida Gélida, Bloco de Gelo, Avalanche, Sopro Glacial, Nevasca, Pena Cortante, Vendaval, Tempestade, Relâmpago, Olho da Tempestade…): uma básica, uma a cada N ataques e uma de fúria. O nome aparece sobre o monstro, anéis vermelhos marcam os alvos durante a preparação e o efeito cai em cada um. Fúria, invocação, congelar e voar continuam.
- **Balanceamento**: medido com `tests/instance_balance.gd` (16 partidas por caso). O que mais matava antes eram os buracos: a cratera sempre embaixo do herói o deixava num poço, acertando a própria parede; agora só os meteoros de fúria abrem uma cratera pequena, e o dano das habilidades foi ajustado para os heróis fracos no nível 5 ficarem como antes, com a entrada livre um pouco mais fácil.
- **Interface menos quadrada**: painéis com cantos bem mais redondos, relevo (luz em cima, sombra embaixo), sombra suave sob painéis e botões, espaços da mochila rebaixados. Molduras PixelLab em `assets/ui/frames/` substituem o desenho sozinhas.
- **Namoro desligado**: a capela fica como cenário.
- **Arte PixelLab**: inventário de tudo que ainda é desenhado por código (126 arquivos: molduras, retratos do POW, efeitos das habilidades, moedas, auras, ícones, tiers de armas, lápide) com prompt, ferramenta e caminho em `assets/pixellab_plan.json` e `docs/PIXELLAB_PLANO.md`. O jogo já carrega cada arquivo quando existir. A geração espera o acesso à API do PixelLab neste ambiente.
